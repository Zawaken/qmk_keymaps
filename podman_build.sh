...
if [ -n "$target" ]; then
	if [ "$(uname)" = "Linux" ] || podman-machine active >/dev/null 2>&1; then
		usb_args="--privileged -v /dev:/dev"
	else
		errcho "Error: target requires podman-machine to work on your platform"
		errcho "See http://gw.tnode.com/podman/podman-machine-with-usb-support-on-windows-macos"
